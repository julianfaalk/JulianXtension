...
// Set tab-cleaner defaults on install (storage.local — superlevels feature).
// NOTE: this `enabled` key lives in storage.local and is the Tab Cleaner master
// switch. Julian's X-dim engine uses its own `enabled` key in storage.sync, so
// the two never collide despite the shared name.
...
// Open the welcome/update page (Julian's settings live in storage.sync so they
// follow the user across devices). The legacy local→sync migration was removed
// during the superlevels merge: superlevels' features legitimately use local
// storage (closed-tab history, redirect/cookie state) and must NOT be pushed
// into the limited sync quota.
